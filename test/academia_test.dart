...
void main() {
  ...
  group('OOP Domain Entity Models & Polymorphism Tests', () {
    test('Permission entity encapsulates properties and equality check', () {
      const p1 = Permission(id: 1, permissionKey: 'students:view', module: 'student', description: 'View students');
      const p2 = Permission(id: 1, permissionKey: 'students:view', module: 'student', description: 'Different description');
      const p3 = Permission(id: 2, permissionKey: 'students:create', module: 'student');

      expect(p1, equals(p2));
      expect(p1, isNot(equals(p3)));
      expect(p1.toJson()['permission_key'], equals('students:view'));
      expect(p1.toJson()['module'], equals('student'));
    });

    test('Role entity encapsulates assigned permissions and hasPermission query', () {
      const pView = Permission(id: 1, permissionKey: 'students:view', module: 'student');
      const pEdit = Permission(id: 2, permissionKey: 'students:edit', module: 'student');
      final teacherRole = Role(id: 2, roleName: 'Teacher', permissions: [pView, pEdit]);

      expect(teacherRole.hasPermission('students:view'), isTrue);
      expect(teacherRole.hasPermission('students:edit'), isTrue);
      expect(teacherRole.hasPermission('students:delete'), isFalse);
      expect(teacherRole.permissions.length, equals(2));
    });

    test('User.create polymorphically instantiates specialized subclasses', () {
      final sa = User.create(
        id: 1,
        username: 'admin',
        roles: [const Role(id: 1, roleName: 'Super Admin')],
      );
      expect(sa, isA<SuperAdminUser>());
      expect(sa.isSuperAdmin, isTrue);
      expect(sa.hasPermission('any_random_permission_key'), isTrue);
      ...
      final teacher = User.create(
        ...
        roles: [
          Role(
            id: 2,
            roleName: 'Teacher',
            permissions: [const Permission(permissionKey: 'lectures:view', module: 'lecture')],
          )
        ],
      );
      expect(teacher, isA<TeacherUser>());
      expect((teacher as TeacherUser).assignedSubject, equals('Physics'));
      expect(teacher.academicQualification, equals('M.Sc Physics'));
      expect(teacher.hasPermission('lectures:view'), isTrue);
      expect(teacher.hasPermission('system:admin'), isFalse);

      final student = User.create(
        id: 3,
        username: 'alice',
        accessGrade: 'Grade 10',
        roles: [const Role(id: 3, roleName: 'Student')],
      );
      expect(student, isA<StudentUser>());
      expect((student as StudentUser).gradeLevel, equals('Grade 10'));

      final parent = User.create(
        id: 4,
        username: 'bob_parent',
        roles: [const Role(id: 4, roleName: 'Parent')],
      );
      expect(parent, isA<ParentUser>());
    });

    test('RbacManager.getUserById returns fully hydrated OOP User entity', () {
      ...
      final regRes = registerUser(
        ...
        adminUsername: 'admin1',
        adminPassword: 'adminpass123',
        accessSubject: 'Mathematics',
        qualificationGrade: 'Ph.D Math',
      );
      final teacherId = regRes['userId'] as int;

      final teacherObj = rbac.getUserById(teacherId);
      expect(teacherObj, isNotNull);
      expect(teacherObj, isA<TeacherUser>());
      expect(teacherObj!.username, equals('teacher_oop'));
      expect(teacherObj.roleNames, contains('Teacher'));
      expect(teacherObj.roles.first.permissions, isNotEmpty);
    });
  });

  group('JWT Token Embedded Permissions Tests', () {
    test('Token generated on login contains embedded permissions array', () {
      registerUser(db, 'admin1', 'adminpass123');
      final regRes = registerUser(
        db,
        'teacher_token_user',
      ...
      final session = loginUser(db, 'teacher_token_user', 'pass123')!;
      expect(session.authToken, isNotNull);
      expect(session.permissions, contains('students:create'));

      // Verify token decoding returns the embedded permissions
      final verified = verifyAuthToken(session.authToken!);
      expect(verified, isNotNull);
      expect(verified!.permissions, contains('students:create'));
      expect(verified.hasPermission('students:create'), isTrue);
      expect(verified.hasPermission('non_existent:action'), isFalse);
    });

    test('Super Admin token grants wildcard permission via hasPermission', () {
      registerUser(db, 'super_admin_jwt', 'adminpass123');
      final session = loginUser(db, 'super_admin_jwt', 'adminpass123')!;

      final verified = verifyAuthToken(session.authToken!);
      expect(verified, isNotNull);
      expect(verified!.isSuperAdmin, isTrue);
      expect(verified.hasPermission('students:delete'), isTrue);
      expect(verified.hasPermission('random_future_module:execute'), isTrue);
    });
  });

  group('Static JSON Permissions Manifest Tests', () {
    test('loadPermissionsManifest reads valid JSON structure from lib/config/permissions.json', () {
      final manifest = loadPermissionsManifest();
      expect(manifest, isNotNull);
      expect(manifest['modules'], isA<List>());

      final modules = manifest['modules'] as List;
      expect(modules.length, greaterThanOrEqualTo(5));

      final moduleNames = modules.map((m) => m['name']).toList();
      expect(moduleNames, contains('Student Module'));
      expect(moduleNames, contains('Teacher Module'));
      expect(moduleNames, contains('Lecture Module'));

      expect(manifest['defaultRoleTemplates'], isNotNull);
      final roleTemplates = manifest['defaultRoleTemplates'] as Map;
      expect(roleTemplates.containsKey('Teacher'), isTrue);
      expect(roleTemplates.containsKey('Student'), isTrue);
      expect(roleTemplates.containsKey('Parent'), isTrue);
    });

    test('initializeSchema populates database permissions table from manifest', () {
      final permsCount = db.select('SELECT COUNT(*) as count FROM permissions;').first['count'] as int;
      expect(permsCount, greaterThanOrEqualTo(20));

      final studentViewPerm = db.select('SELECT * FROM permissions WHERE permission_key = ?;', ['students:view']);
      expect(studentViewPerm, isNotEmpty);
      expect(studentViewPerm.first['module'], equals('Student Module'));
    });
  });
}
