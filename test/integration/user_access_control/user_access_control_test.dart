...
void main() {
  group('1. User and Access Control Integration Tests', () {
    ...
    test('Test 1.3: Direct Custom Permissions', () {
      ...
      // Initially user does not have users:edit permission
      expect(checkUserPermission(db, userId, 'users:edit'), isFalse);

      // Grant direct permission ID 15 (users:edit)
      assignDirectPermissionToUser(db, userId: userId, permissionId: 15, adminUsername: 'superadmin', adminPassword: 'adminpass');

      // Verify direct permission now evaluates to true
      expect(checkUserPermission(db, userId, 'users:edit'), isTrue);
    });
