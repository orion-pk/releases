...
void main() {
  group('2. Student Module Integration Tests', () {
    ...
    test('Test 2.4: Grades and Report Card Permissions', () {
      ...
      final permCheck = checkUserPermission(db, studentId, 'read:own_data');
      expect(permCheck, isFalse); // Default user before role assignment

      assignRoleToUser(db, userId: studentId, roleId: studentRoleId, adminUsername: 'superadmin', adminPassword: 'adminpass');
      expect(checkUserPermission(db, studentId, 'read:own_data'), isTrue);
    });
  });
}
