...
for ($i = 0; $i -lt $serverContent.Count; $i++) {
    if ($serverContent[$i] -like 'const String academyCurrentVersion*' -or $serverContent[$i] -like 'const String ACADEMY_CURRENT_VERSION*') {
        $serverContent[$i] = "const String academyCurrentVersion = '$Version';"
    }
}
...
Write-Host ""
Write-Host "[2/6] Committing changes to main..." -ForegroundColor Yellow
git add .
git commit -m "feat(release): release version $Version" --allow-empty
git push origin main
Write-Host "OK - Changes committed and pushed to main" -ForegroundColor Green
...
Write-Host ""
Write-Host "[6/6] Pushing Tag Snapshot and Publishing Release to GitHub..." -ForegroundColor Yellow
$notes = "Academy Platform Release $TagName"

# Create clean tree commit and tag on orion-pk/releases so GitHub auto-generated zips contain actual Academia code
$TreeId = (git write-tree).Trim()
$CleanCommitId = (git commit-tree $TreeId -m "Release $TagName source snapshot").Trim()
git tag -f $TagName $CleanCommitId
git push https://github.com/orion-pk/releases.git "$($TagName):$($TagName)" --force
